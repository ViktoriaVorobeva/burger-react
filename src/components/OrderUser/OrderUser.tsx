...
import { TIngridient, TOrder } from '../../types/ingridient';
...
export const OrderUser = () => {
    ...
    const orderIngredientsForImage = ingredients.filter((ingredient: TIngridient) => order?.ingredients.includes(ingredient._id))

    const orderIngredients =
        order?.ingredients.map((id: string) => {
            return ingredients.find((item: TIngridient) => item._id === id);
        }).filter(inNotUndefined);

    const totalOrderPrice = orderIngredients?.reduce(
        (acc: number, ingredient: TIngridient) => acc + ingredient.price,
        0
    );
                    ...
                    .map((item: TIngridient) =>
                        <li className={styles.item} key={item._id}>
                            ...
                            <p className={`${styles.price} text text_type_digits-default`}>
                                {orderIngredients?.filter((i: TIngridient) => i._id === item._id).length} x {item.price} <CurrencyIcon type='primary' /></p>
                        </li>
                    )}
